...
sc_obj$main_cluster_int <- as.character(Idents(sc_obj))

# Make a umap which shows the cells whose annotation is different between integrated and non-integrated
sc_obj@meta.data <- sc_obj@meta.data %>%
  mutate(annotation_changed=(main_cluster!=main_cluster_int))

sink("statistics.txt")
print("Number of cells whose annotations changed due to integration")
table(sc_obj$annotation_changed)
sink(NULL)

# .......... UMAP of changed annotation
plot <- DimPlot(sc_obj, group.by="annotation_changed", reduction="umap", label=TRUE, repel=TRUE, pt.size=0.25, cols=c("#bababa", "#ca0020"), order=c(TRUE)) +
  theme(legend.position="none")
filename <- "umap_annotation_changed.pdf"
ggsave(plot=plot, filename=filename, width=7, height=7)
plot <- DimPlot(sc_obj, group.by="annotation_changed", reduction="umap", pt.size=0.25, cols=c("#bababa", "#ca0020"), order=c(TRUE)) +
  theme(legend.position="none")
filename <- "umap_annotation_changed.png"
ggsave(plot=plot, filename=filename, width=7, height=7)

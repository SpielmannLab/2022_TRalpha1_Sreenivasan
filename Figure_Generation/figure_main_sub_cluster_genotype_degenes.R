...
geo_metadata <- sc_obj@meta.data %>%
  select(all_of(c(paste0("RNA_snn_res.",res),
    "main_cluster",
    "main_sub_cluster",
    "genotype",
    "rep")))
...
Idents(sc_obj) <- paste0(sc_obj$genotype, "_", sc_obj$main_sub_cluster)
...
for(main_sub_cluster in main_sub_clusters){
  ident1 <- paste0("tra1_", main_sub_cluster)
  ident2 <- paste0("wt_", main_sub_cluster)
  #compute markers for tra1
  markers_tra1 <- FindMarkers(sc_obj, ident.1=ident1, ident.2=ident2, logfc.threshold = 0.5, min.pct = min_pct, test.use = "wilcox", min.cells.group=0.5, only.pos=TRUE)
  if(NROW(markers_tra1)!=0){markers_tra1$genotype <- "tra1"}
  #compute markers for wt
  markers_wt <- FindMarkers(sc_obj, ident.1=ident2, ident.2=ident1, logfc.threshold = 0.5, min.pct = min_pct, test.use = "wilcox", min.cells.group=0.5, only.pos=TRUE)
  if(NROW(markers_wt)!=0){markers_wt$genotype <- "wt"}

  markers <- rbind(markers_tra1, markers_wt)
  if(NROW(markers)!=0){
...
filename <- paste0("markers_all_genotype_min_pct_0.1_",
  DefaultAssay(sc_obj),
  ".tsv")
...
filename <- paste0("no_of_genotype_DEgenes_per_main_sub_cluster_",
  DefaultAssay(sc_obj),
  ".pdf")
p <- ggplot(data=all_markers, aes(x=fct_infreq(main_sub_cluster), fill=genotype)) +
...
filename <- paste0("Output_Neuron_genotype_markers_",
  DefaultAssay(sc_obj),
  ".txt")
sink(filename, append=TRUE)
print("Distinct neuronal de genes across genotype")
neurons_markers %>%
  .$gene %>%
  table
sink(NULL)
...
filename <- paste0("markers_frequent_genotype_min_pct_0.1_",
  DefaultAssay(sc_obj),
  ".tsv")
